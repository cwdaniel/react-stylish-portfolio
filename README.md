...
After installation, run `npm install` and then run `parcel index.html --open ` which will open up a preview of the template in your default browser, watch for changes to core template files, and live reload the browser when changes are saved. You can view the `package.json` to see which tasks are included with the dev environment.
